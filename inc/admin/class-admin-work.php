...
class Admin_Work {
	...
	private function __construct() {
		...
		if(is_admin()) {
			...
			add_action( 'manage_work_posts_custom_column', [ $this, 'custom_columns_value' ], 2, 2 );
			add_filter( 'manage_work_posts_columns', [ $this, 'add_custom_columns_header' ] );

			add_action( 'edit_form_before_permalink', [$this, 'thumbnail_title_field'] );

			if(unyson_exists()) {
				add_action( 'fw_save_post_options', [$this, 'save_work_15'], 15, 2 );
			} else {
				add_action( 'save_post_work', [$this, 'save_work_15'], 15, 2 );
			}
	...
	public function save_work_15($post_id, $post) {
		...
		if ( ! current_user_can( 'edit_work', $post_id ) ) {
			return;
		}
	...
	public function custom_columns_value($column, $post_id) {
		switch ($column) {
			case 'gurl':
				echo esc_html(fw_get_db_post_option($post_id, 'work_zalo'));
				break;
	...
	public function enqueue_scripts($hook) {
		global $post_type;
		if(($hook=='post.php' || $hook=='edit.php') && $post_type=='work') {
			wp_enqueue_style( 'work', THEME_URI.'/assets/css/admin-work.css', [], '' );
			//wp_enqueue_script( 'work', THEME_URI.'/assets/js/admin-work.js', array('jquery'), '', false );
		}
	}
