...
class Admin_Estimate {
	...
	private function __construct() {
		...
		if(is_admin()) {
			...
			add_action( 'manage_estimate_posts_custom_column', [ $this, 'custom_columns_value' ], 2, 2 );
			add_filter( 'manage_estimate_posts_columns', [ $this, 'add_custom_columns_header' ] );
	...
	public function switch_boxes() {
        ...
        ,   'estimate'              // Screen, we use all screens with meta boxes.
        ,   'normal'          // Context
        ,   'core'            // Priority
        );
	}
    ...
    ?>
        <label class="screen-reader-text" for="excerpt">URL dự toán</label>
        <input type="text" id="excerpt" name="excerpt" value="<?php echo esc_attr( $post->post_excerpt ); ?>">
        <?php
    }
