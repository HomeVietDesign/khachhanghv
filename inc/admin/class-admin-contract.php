...
class Admin_Contract {
	...
	private function __construct() {
		...
		if(is_admin()) {
			...
			// add_action( 'manage_contract_posts_custom_column', [ $this, 'custom_columns_value' ], 2, 2 );
			// add_filter( 'manage_contract_posts_columns', [ $this, 'add_custom_columns_header' ] );

			add_action( 'edit_form_before_permalink', [$this, 'thumbnail_title_field'] );

			if(unyson_exists()) {
				add_action( 'fw_save_post_options', [$this, 'save_contract_15'], 15, 2 );
			} else {
				add_action( 'save_post_contract', [$this, 'save_contract_15'], 15, 2 );
			}
		}

	}

	public function save_contract_15($post_id, $post) {
		if ($post->post_type!='contract') return;
		...
		if ( ! current_user_can( 'edit_contract', $post_id ) ) {
			return;
		}
	...
	public function switch_boxes() {
		...
		remove_meta_box(
            'postexcerpt' // ID
        ,   'contract'            // Screen, empty to support all post types
        ,   'normal'      // Context
        );
        ...
        ,   'contract'              // Screen, we use all screens with meta boxes.
        ,   'normal'          // Context
        ,   'core'            // Priority
        );
        */
