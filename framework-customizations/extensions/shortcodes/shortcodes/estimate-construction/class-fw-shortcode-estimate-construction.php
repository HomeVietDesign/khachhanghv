...
class FW_Shortcode_Estimate_Construction extends FW_Shortcode
{
	...
	public function ajax_estimate_contractor_construction_hide() {
		...
		if(current_user_can('estimate_construction_edit') && $current_client && $contractor_id && check_ajax_referer( 'global', 'nonce', false )) {
			...
			fw_set_db_term_option($current_client->term_id, 'passwords', 'contractor_construction_hide', $contractor_hide);
			$response = true;
		}
	...
	public function ajax_update_estimate_construction() {
		...
		if(current_user_can( 'estimate_construction_edit' ) && check_ajax_referer( 'edit-estimate-construction', 'nonce', false )) {
			...
			if($estimate_construction_client && $estimate_construction_contractor) {
				...
				if(empty($estimates)) $estimates = [];
				$estimate = isset($estimates[$estimate_construction_client])?$estimates[$estimate_construction_client]:[ 'required'=>'', 'received'=>'', 'completed'=>'', 'sent'=>'', 'value'=>'', 'unit'=>'', 'zalo'=>'', 'info'=>'', 'link'=>'', 'attachment_id'=>''];
	...
	public function ajax_get_edit_estimate_construction_form() {
		...
		if($client && $contractor) {
			...
			<form id="frm-edit-estimate-construction" method="POST" action="" >
				...
				<div class="col mb-3">
					<input type="text" id="estimate_construction_value" name="estimate_construction_value" placeholder="Giá trị" class="form-control" value="<?php echo esc_attr($estimate['value']); ?>">
				</div>
				<div class="col mb-3">
					<input type="text" id="estimate_construction_unit" name="estimate_construction_unit" placeholder="Ghi chú" class="form-control" value="<?php echo esc_attr($estimate['unit']); ?>">
				</div>
				<div class="col mb-3">
			...
			<div class="estimate estimate-<?=$contractor_id?> border border-dark h-100 bg-black">
				...
				<div class="contractor-thumbnail position-relative">
					...
					<div class="position-absolute bottom-0 end-0 m-1 d-flex">
						...
						<button class="estimate-contractor-construction-hide btn btn-sm btn-danger text-yellow ms-2" type="button" data-client="<?=$client->term_id?>" data-contractor="<?=$contractor_id?>" data-contractor-title="<?php echo esc_attr(get_the_title( $contractor_id )); ?>"><span class="dashicons dashicons-visibility"></span></button>
